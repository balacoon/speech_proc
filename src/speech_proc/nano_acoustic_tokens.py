"""
Copyright 2025 Balacoon

Acoustic token extraction using NeMo AudioCodecModel
"""

import argparse
import os
from typing import Optional

import numpy as np
import torch
import tqdm

from speech_proc.audio_dir import AudioDir


def extract_tokens(
    audio_dir_path: str,
    output_dir: str,
    batch_size: int = 16,
    max_dur: Optional[float] = None,
    device: str = "cpu",
    model_name: str = "nvidia/nemo-nano-codec-22khz-1.89kbps-21.5fps",
):
    """
    Extract acoustic tokens from audio files using NeMo AudioCodecModel.

    Args:
        audio_dir_path: Path to directory containing audio files
        output_dir: Path to directory where npz files will be saved
        batch_size: Number of audio files to process at once
        max_dur: Maximum duration in seconds, files longer than this are skipped
        device: Device to run model on ('cpu' or 'cuda')
        model_name: NeMo model name or path to checkpoint
    """
    # Create output directory
    os.makedirs(output_dir, exist_ok=True)

    # Load model
    print(f"Loading model {model_name}...")
    from nemo.collections.tts.models import AudioCodecModel

    model = AudioCodecModel.from_pretrained(model_name).eval().to(device)
    model_sample_rate = model.sample_rate
    print(f"Model sample rate: {model_sample_rate}")

    # Initialize audio directory
    audio_dir = AudioDir(audio_dir_path)
    audio_ids = audio_dir.get_ids(
        # allow any sample rate starting from 16000Hz
        to_sort=True,
        expected_sample_rate=None,
        max_dur=max_dur,
    )
    print(f"Found {len(audio_ids)} audio files")

    # Process audio files in batches
    num_batches = (len(audio_ids) + batch_size - 1) // batch_size

    for batch_idx in tqdm.tqdm(range(num_batches), desc="Processing batches"):
        start_idx = batch_idx * batch_size
        end_idx = min(start_idx + batch_size, len(audio_ids))
        batch_ids = audio_ids[start_idx:end_idx]

        # Load and prepare audio for this batch
        batch_audios = []
        batch_lengths = []

        for audio_id in batch_ids:
            try:
                # Read audio using AudioDir
                audio_data, sample_rate = audio_dir.read(
                    audio_id, dtype="float32", sample_rate=model_sample_rate
                )

                # Convert to tensor
                audio_tensor = torch.from_numpy(audio_data)
                batch_audios.append(audio_tensor)
                batch_lengths.append(audio_tensor.shape[0])

            except Exception as e:
                print(f"Error loading audio {audio_id}: {e}")
                continue

        if not batch_audios:
            continue

        # Pad audios to the same length
        max_length = max(batch_lengths)
        padded_audios = []
        for audio in batch_audios:
            if audio.shape[0] < max_length:
                padding = torch.zeros(max_length - audio.shape[0])
                audio = torch.cat([audio, padding], dim=0)
            padded_audios.append(audio.unsqueeze(0))  # Add batch dimension

        # Stack into batch tensor (batch_size x samples)
        batch_audio_tensor = torch.cat(padded_audios, dim=0).to(device)
        batch_lengths_tensor = torch.tensor(batch_lengths, dtype=torch.long).to(device)

        # Run model encoding
        with torch.no_grad():
            encoded_tokens, encoded_len = model.encode(
                audio=batch_audio_tensor, audio_len=batch_lengths_tensor
            )

        # Process outputs for each audio file in the batch
        for i, audio_id in enumerate(batch_ids):
            # Get tokens for this sample
            actual_token_length = encoded_len[i].cpu().item()
            tokens = (
                encoded_tokens[i, :, :actual_token_length]
                .cpu()
                .numpy()
                .astype(np.int32)
            )

            # Save to npz file
            output_path = os.path.join(output_dir, f"{audio_id}.npz")
            np.savez(output_path, acoustic_tokens=tokens)

    print("Acoustic token extraction complete!")


def main():
    parser = argparse.ArgumentParser(
        description="Extract acoustic tokens from audio files using NeMo AudioCodecModel"
    )
    parser.add_argument(
        "audio_dir", type=str, help="Path to directory containing audio files"
    )
    parser.add_argument(
        "output_dir", type=str, help="Path to directory where npz files will be saved"
    )
    parser.add_argument(
        "--batch-size",
        "-b",
        type=int,
        default=16,
        help="Number of audio files to process at once (default: 16)",
    )
    parser.add_argument(
        "--max-dur",
        "-m",
        type=float,
        default=None,
        help="Maximum duration in seconds, files longer than this are skipped",
    )
    parser.add_argument(
        "--device",
        "-d",
        type=str,
        default="cpu",
        choices=["cpu", "cuda"],
        help="Device to run model on (default: cpu)",
    )
    parser.add_argument(
        "--model",
        type=str,
        default="nvidia/nemo-nano-codec-22khz-1.89kbps-21.5fps",
        help="NeMo model name or path to checkpoint (default: nvidia/nemo-nano-codec-22khz-1.89kbps-21.5fps)",
    )

    args = parser.parse_args()

    extract_tokens(
        audio_dir_path=args.audio_dir,
        output_dir=args.output_dir,
        batch_size=args.batch_size,
        max_dur=args.max_dur,
        device=args.device,
        model_name=args.model,
    )


if __name__ == "__main__":
    main()
